      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {events.slice(0, 6).map((ev: any, i: number) => {
            ...
            const { Icon } = cfg;
            const hasLink = Boolean(ev.link_url);

            const inner = (
              <motion.div
                whileHover={{ y: -3 }}
                transition={{ duration: 0.25 }}
                className={`group rounded-2xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.05] transition-colors p-5 flex flex-col gap-3 h-full relative overflow-hidden ${hasLink ? "cursor-pointer" : ""}`}
              >
                ...
                {hasLink && (
                  <div className="flex items-center justify-between mt-1">
                    <span className="text-xs text-white/40 group-hover:text-white transition-colors flex items-center gap-1.5">
                      Read article <ArrowUpRight className="h-3.5 w-3.5 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 transition-transform" />
                    </span>
                  </div>
                )}
              </motion.div>
