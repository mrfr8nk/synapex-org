...
import {
  ...
  ArrowRight, CheckCircle, User, MapPin, Link2, FileText, Loader2,
} from "lucide-react";
  ...
  async function loadProfile(user: any) {
    try {
      ...
      if (data) {
        ...
        setForm((f) => ({
          ...f,
          name: meta.full_name || meta.name || meta.login || "",
              ...
              <FadeIn direction="up">
                <div className="text-center mb-16">
                  ...
                  <p className="mt-5 text-lg text-white/50 max-w-xl mx-auto leading-relaxed">
                    We're a global crew of engineers, designers and builders who ship real software. Any age, any country — your code is what matters.
                  </p>
                </div>
              </FadeIn>
              ...
              <div className="grid lg:grid-cols-2 gap-10 items-start">
                ...
                <FadeIn direction="right">
                  <div className="rounded-3xl border border-white/10 bg-white/[0.03] backdrop-blur-xl p-8 space-y-4">
                    ...
                    <p className="text-sm text-white/45">Sign in with your existing account. No password needed.</p>
                ...
                <div className="text-center mb-10">
                  {session.user.user_metadata?.avatar_url && (
                    <img
                      ...
                      alt=""
                      className="h-16 w-16 rounded-full border border-white/15 mx-auto mb-4 object-cover"
                    />
                  )}
              ...
              <FadeIn direction="up" delay={0.05}>
                <form onSubmit={saveProfile} className="rounded-3xl border border-white/10 bg-white/[0.03] p-8 space-y-5">
                  <div className="space-y-1.5">
                    <label className="text-[11px] uppercase tracking-[0.2em] text-white/40 flex items-center gap-1.5">
                      <User className="h-3 w-3" /> Full name *
                    </label>
                    <input required value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-3 text-sm focus:border-white/40 outline-none transition-colors" />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea value={form.bio} onChange={(e) => setForm((f) => ({ ...f, bio: e.target.value }))}
                      rows={3} placeholder="Tell us about yourself, what you build, what you're passionate about..."
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-3 text-sm focus:border-white/40 outline-none transition-colors resize-none placeholder:text-white/20" />
                  </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <input value={form.location} onChange={(e) => setForm((f) => ({ ...f, location: e.target.value }))}
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-3 text-sm focus:border-white/40 outline-none transition-colors placeholder:text-white/20" />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[11px] uppercase tracking-[0.2em] text-white/40 flex items-center gap-1.5">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[11px] uppercase tracking-[0.2em] text-white/40 flex items-center gap-1.5">
                      <Link2 className="h-3 w-3" /> Portfolio URL
                    </label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[11px] uppercase tracking-[0.2em] text-white/40">Skills (pick all that apply)</label>
                  ...
                  <button
                    type="submit"
                    disabled={saving || saved || !form.name}
                    className="w-full flex items-center justify-center gap-2 rounded-full bg-white text-black py-3 text-sm font-medium hover:bg-white/90 transition-colors disabled:opacity-60"
                  >
